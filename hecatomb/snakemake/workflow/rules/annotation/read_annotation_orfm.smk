"""
This is an alt version of read_annotation.smk to allow better hit rates with new versions of MMSEQs.
It is not currently implemented as it's a bit hacky:
 - ORFM to convert seqtable.fasta to ORFs
 - ORFs for eash seqtable seqID are STITCHED TOGETHER (not ideal but necessary for generating the bigtable at the end)
 - stitched ORFs processed through easy-taxonomy like normal
"""

rule orfm_seqtable:
    """Run ORFM on the seqtable"""
    input:
        os.path.join(dir.out.results,"seqtable.fasta")
    output:
        temp(os.path.join(dir.out.temp,"seqtable.orfm.fasta.temp"))
    shell:
        """orfm {input} | sed 's/_[0-9]*_[0-9]*_[0-9]*//' > {output}"""


rule orfm_seqtable_filter:
    """Pull the longest ORF for each seqtable seq"""
    input:
        os.path.join(dir.out.temp,"seqtable.orfm.fasta.temp")
    output:
        temp(os.path.join(dir.out.temp,"seqtable.orfm.fasta"))
    log:
        os.path.join(dir.out.stderr, "orfm_seqtable_filter.stderr")
    script:
        os.path.join(dir.scripts, "orfmFilt.py")
...
rule PRIMARY_AA_taxonomy_assignment:
    ...
    input:
        seqs=os.path.join(dir.out.temp,"seqtable.orfm.fasta"),
...
rule SECONDARY_AA_taxonomy_assignment:
    ...
    shell:
        ...
        mmseqs easy-taxonomy {input.seqs} {input.db} {params.alnRes} {params.tmppath} \
            {params.filtaa} {params.sensaa} {params.formataa} \
            --threads {threads} --split-memory-limit {params.memsplit} &> {log}
        sort -k1,1 -k3,3g {output.aln} > {output.alnsort} 2>> {log}
        rm {log}
        """
...
rule SECONDARY_AA_tophit_lineage:
    ...
    input:
        db=dir.dbs.taxonomy,
        tophit=os.path.join(dir.out.secondaryAA,"MMSEQS_AA_SECONDARY_tophit_aln_sorted")
    output:
...
rule SECONDARY_AA_generate_output_table:
    ...
    input:
        aln=os.path.join(dir.out.secondaryAA,"MMSEQS_AA_SECONDARY_tophit_aln_sorted"),
