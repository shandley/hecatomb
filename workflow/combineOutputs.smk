...
allDirSmplLen = {}  # config["hecatomb"]["args"]["output_paths"]["results"]:sample:len
allSamples = []
assemblyFiles = True
...
targets = [
    ...
    os.path.join(config["hecatomb"]["args"]["output_paths"]["results"], "assembly.fasta"),
...
rule combineAssemblies:
    ...
    output:
        assembly = os.path.join(config["hecatomb"]["args"]["output_paths"]["results"], 'assembly.fasta'),
        contigs = temp(os.path.join(config["hecatomb"]["args"]["output_paths"]["results"], 'allContigs.fa')),
        flye = temp(directory(os.path.join(config["hecatomb"]["args"]["output_paths"]["results"], 'Flye')))
    params:
        os.path.join(config["hecatomb"]["args"]["output_paths"]["results"], 'Flye', 'assembly.fasta')
    threads:
