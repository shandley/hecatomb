...
rule canu_sample_assembly:
    ...
    input:
        os.path.join(TMPDIR,"p01","{sample}_R1.unmapped.fasta")
    output:
...
rule combine_canu_unassembled:
    ...
    output:
        temp(os.path.join(TMPDIR,"p01","unmappedRescue_R1.unmapped.fasta"))
    shell:
        """cat {input} > {output}"""
...
rule coverage_calculations:
    ...
    input:
        r1 = os.path.join(TMPDIR,"p01","{sample}_R1.unmapped.fasta"),
